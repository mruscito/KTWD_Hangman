...
class Hangman(object):
    ...
    # check that the word is under 9 characters using len()
    # create variable 'letters' = list(word)
...
'''
    ...
    def lost(wrong_guesses, game_over=False):
        if wrong_guesses == 6:
            game_over = True
            window.blit(title_font.render('You Lose', True, (255,0,0)), (145, 432))
        return game_over
    ...
    def win(letters, wrong_guesses):
        if len(letters) == 0:
            game_over = True
            window.blit(title_font.render('You Win!', True, (50, 205, 50), (145, 432)))
            if wrong_guesses > 0:
